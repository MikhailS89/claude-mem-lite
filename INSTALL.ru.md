...
### Есть ли от плагина польза

```bash
node $m stats --since 7d       # текущий проект за неделю; --all — по всем проектам
```

`stats` не оценивает на глаз, а считает по транскриптам сессий:
- получил ли Claude recap и какого размера;
- сколько было подсказок по файлам;
- сколько раз он сам искал в памяти (`mem-search`);
- как начиналась работа: сколько вызовов инструментов до первой правки файла проекта, и сколько из них — `git log` / `show` / `blame`.

Сессии с recap сравниваются с сессиями без него. В сравнение входят и транскрипты до установки плагина (Claude Code хранит их 30 дней). Здесь же — сколько стоили сводки по коммитам.

Меньше вызовов до первой правки и меньше походов в git history с recap — это и есть эффект. Группы различаются не только recap'ом (у первой сессии в проекте его нет), поэтому смотрите на тенденцию за несколько недель, а не на точное число.
